...
"+"\n"+"\
Leave search field blank to retrieve your own current IP."+"\n"+"\
"+"\n"+"\
...
//global variables..
let url = '';
let ipgeoKey = '5b7fb872d30843b3a61f0bb5c13c5e89'; //.length== 9 
...
let wisxApiUrl="https://www.whoisxmlapi.com/whoisserver/DNSService?apiKey=at_sqEwNRG69xcfzaEFQMPOcvpWRsTgB&type=A&outputFormat=JSON&domainName="
...
//Setup Animations for the spinning arrow.
 let iconArrowBtn = document.getElementById('iconArrowBtn');
  ...
  [{transform: 'rotate(0deg)'},{transform: 'rotate(10800deg)'}],// Keyframes
  {duration: 6000} // Keyframe settings            3600
  );
    ...
    if (srchInpTxt.value===""||inpTxtHasIp){//inpTxtHasIp domain must be converted to an ip address First via whoisxmlapi.com!
        ...
        //Cookie.set('SameSite', 'Lax');//let`s get rid of that pesky samesite=lax browser error..
         ...
         // console.error(Error); 
           ...
           //If the message is "Failed to load resource: net::ERR_BLOCKED_BY_CLIENT" then
           //check if Adblockers\Shields is disabled in Chrome and try again.
       })
     }
}
