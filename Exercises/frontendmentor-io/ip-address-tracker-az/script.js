...
let dnsResUrl = 'https://ipaddresstracker.azurewebsites.net/dnsRes/' // used for finding ip address for Domain names entered..
...
// },5000);// 5 seconds wait, which can be cancelled when data is returned..
   ...
   if (srchInpTxt.value===""){//Return default ip info, nothing is in the search field..
    ...
    //  console.log("value has No data-- :.."+srchInpTxt.value);
          url = 'https://ipaddresstracker.azurewebsites.net/ipgeoApi/'
       console.log(url);
       errMsgSite = url;
        ...
        // for regex check https://stackoverflow.com/questions/41942690/removing-http-or-http-and-www/41942787
         ...
         srchInpTxtcleaned = srchInpTxtcleaned.split(' ').join('');
            ...
            let chkDomIpvalid = srchInpTxtcleaned.split("."); 
              url = 'https://ipaddresstracker.azurewebsites.net/ipgeoApi/'+srchInpTxtcleaned;
          ...
          //Check if a domain name has been entered. more than 0, less than 4 Octets is a domain..
            if (chkDomIpvalid.length > 0 && chkDomIpvalid.length < 4 ){ 
                ...
                function is_domain(srchInpTxtcleaned){
                  errMsgSite = 'https://ipaddresstracker.azurewebsites.net/dnsRes';
      ...
      if (dnsdata){//We have some dnsdata!!
        ...
        //console.log(data.DNSData.dnsRecords[0].address);
          ...
          //console.log("Tracker 6");
                inpTxtHasIp = true;
                url = 'https://ipaddresstracker.azurewebsites.net/ipgeoApi/'+dnsdata[0];
                console.log(url);
...
console.log("Tracker 8")     
  ...
  async function getmap(lat,lng,width,height){
      ...
      errMsgSite = "https://ipaddresstracker.azurewebsites.net/hereMApi";
      
      
      let mapurl = 'https://ipaddresstracker.azurewebsites.net/hereMApi/&c='+lat+','+lng+'&u=250&z=14&ppi=72&f=0&h='+height+'&w='+width;
             ...
             if (response.ok){
               ...
               setTimeout(()=>{
                let timestamp = new Date().getTime();
                    ...
                    if (window.innerWidth>'700'){
                        document.getElementById("HereApiMapDskt").src='https://ipaddresstracker.azurewebsites.net/hImgMaps/hereMap0.png?t='+timestamp;//append time to force it refresh the cache..
                      }else{document.getElementById("HereApiMapMble").src='https://ipaddresstracker.azurewebsites.net/hImgMaps/hereMap0.png?t='+timestamp;//append time to force it refresh the cache..
                      }
